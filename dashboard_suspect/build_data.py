...
def load_sick() -> pd.DataFrame:
    ...
    if "gnl_nm_label" in df.columns:
        ...
        df.loc[df["drug_key"].str.contains("콜린", na=False), "drug_key"] = "choline_alfoscerate"
    return df
...
def main() -> None:
    ...
    payload = {
        ...
        "timeline": TIMELINE,
        "notes": [
            "8종 의심 의약품 + 콜린알포세레이트의 cmpn × 월 × 상병 시계열 (HIRA OpenAPI cmpn_sick 트랙).",
            "건강보험·조제기준만 (insupTp=4, cpmdPrscTp=01). 의료급여·보훈·처방기준 별도.",
            "퇴출 약물(옥시라세탐·아세틸엘카르니틴)은 2024년 청구가 0에 가까운 것이 정상.",
